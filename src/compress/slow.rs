            ...
            'outer: while symbols.len() < 16384 && ip + 16 < data.len() {
                ...
                if length == 0 {
                    ...
                    (length, distance, match_start) = self
                        .match_finder
                        .get_and_insert(&data, ip, current, 4);
                }
                ...
                if length >= 3 {
                        ...
                        && ip + length as usize + 9 <= data.len()
                    {
                        ...
                        let (next_length, next_distance, next_match_start) =
                            self.match_finder.get_and_insert(&data, ip, current >> 8, length + 1);
                    ...
                    for j in (insert_start..insert_end).step_by(3) {
                        ...
                        self.match_finder.insert(data, v >> 8, j + 1);
                        self.match_finder.insert(data, v >> 16, j + 2);
                    }
